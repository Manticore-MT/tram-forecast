const { Card, Badge, Button, Icon, Stat, LoadMeter, Switch, Tabs, Tag, Tooltip } = window.DesignSystem_8e46b6;
...
function MapCanvas({ hour, onPick, active }) {
  ...
  React.useEffect(() => {
    ...
    const map = window.L.map(el, { zoomControl: false, attributionControl: true }).setView([55.779, 37.699], 13);
    ...
    const poll = setInterval(() => {
      sync();
      if (map.getSize().x > 0 && map.getSize().y > 0) clearInterval(poll);
    ...
    return () => { clearInterval(poll); if (ro) ro.disconnect(); window.removeEventListener("resize", sync); };
  }, []);
  React.useEffect(() => {
    ...
    for (let i = 0; i < pts.length - 1; i++) {
      window.L.polyline([pts[i].ll, pts[i + 1].ll], { color: hexFor(pts[i].v), weight: 6, opacity: 0.85, dashArray: "1 0" }).addTo(g);
    }
    pts.forEach((s, i) => {
      window.L.circleMarker(s.ll, { radius: i === active ? 11 : 7, color: "#0E1113", weight: 2, fillColor: hexFor(s.v), fillOpacity: 1 })
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "var(--space-5)" }}>
        <Panel title="Качество данных">
          <div style={{ display: "flex", flexDirection: "column", gap: "var(--space-4)" }}>
            {[["Пропуски валидаций", 0.006], ["Дубли транзакций", 0.002], ["Остановки без привязки", 0.014], ["Выбросы телематики", 0.021]].map(([k, v]) => (
        ...
        <Panel title="API прогноза" action={<Badge tone="info">Spring Boot · Netty</Badge>}>
          <div style={{ display: "flex", flexDirection: "column", gap: "var(--space-2)", font: "var(--type-mono-s)" }}>
            {[["GET", "/api/v1/forecast?route=17&horizon=day", "p95 84 мс"],
              ...
              ["POST", "/api/v1/model/retrain", "async"]].map(([m, p, l]) => (
...
Object.assign(window, { MapView, IngestView, GEO });
