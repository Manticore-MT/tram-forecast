const { Button, Icon, Badge, Stat, LoadMeter, Switch, Select, Card, Tabs, Tooltip } = window.DesignSystem_8e46b6;
...
function Overview({ horizon, route }) {
      ...
      <Panel title="Рекомендации диспетчеру">
        <div style={{ display: "flex", flexDirection: "column", gap: "var(--space-3)" }}>
          {[["danger", "Электрозаводская, 18:30–19:10", "Прогноз 86 % — добавить 2 вагона на выпуск"],
            ...
            ["ok", "Лефортово, весь день", "Резерв: можно снять 1 вагон после 20:00"]].map(([tone, t, d]) => (
            <div key={t} style={{ display: "flex", alignItems: "center", gap: "var(--space-4)", padding: "var(--space-4)", background: "var(--bg-surface-2)", borderRadius: "var(--radius-md)", boxShadow: "var(--inset-hairline)" }}>
              <Badge tone={tone}>{tone === "danger" ? "Пик" : tone === "warn" ? "Внимание" : "Резерв"}</Badge>
...
function ModelView() {
  ...
  return (
    <div style={{ display: "grid", gap: "var(--space-5)" }}>
      <div style={{ display: "grid", gridTemplateColumns: "repeat(4,1fr)", gap: "var(--space-5)" }}>
        {[["MAPE", "7.4", "%"], ["RMSE", "48.2", ""], ["R²", "0.91", ""], ["Обучение", "14", "мин"]].map(([l, v, u]) => (
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "var(--space-5)" }}>
        <Panel title="Вклад признаков">
          <div style={{ display: "flex", flexDirection: "column", gap: "var(--space-4)" }}>
            {[["Час суток", 0.94], ["День недели", 0.71], ["Погода", 0.48], ["События в городе", 0.35], ["Интервал движения", 0.62]].map(([f, v]) => (
        ...
        <Panel title="Обучения" action={<Button size="sm" variant="secondary" iconLeft={<Icon name="refresh-cw" size={14} />}>Переобучить</Button>}>
          <div style={{ display: "flex", flexDirection: "column", gap: "var(--space-3)" }}>
            {[["v14", "13.09 04:00", "ok", "7.4 %"], ["v13", "06.09 04:00", "ok", "8.1 %"], ["v12", "30.08 04:00", "warn", "9.6 %"]].map(([v, d, tone, m]) => (
...
Object.assign(window, { Overview, RouteView, ModelView, STOPS });
