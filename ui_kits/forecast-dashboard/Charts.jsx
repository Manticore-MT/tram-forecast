...
function ForecastChart({ actual, forecast, band, height = 260 }) {
  ...
  return (
    <svg viewBox={`0 0 ${w} ${height}`} preserveAspectRatio="none" style={{ width: "100%", height, display: "block" }}>
      ...
      <g transform={`translate(${w * split},0)`}>
        {upper && <path d={`${path(upper, w * (1 - split), h, max)} L${w * (1 - split)},${h - (lower[lower.length - 1] / max) * h} ${lower.slice().reverse().map((v, i) => `L${w * (1 - split) - (i / (lower.length - 1)) * w * (1 - split)},${h - (v / max) * h}`).join(" ")} Z`} fill="var(--accent)" fillOpacity="0.14" />}
...
function Sparkline({ data, color = "var(--cyan-500)", height = 36 }) {
...
function RouteStrip({ stops, active, onPick }) {
  return (
...
Object.assign(window, { seedSeries, ForecastChart, Sparkline, Heatmap, RouteStrip, LOAD_VARS });
