...
```
merge в main ──► CI собирает образы бэкенда и фронтенда ──► GitHub Container Registry
                                                        │
...
- **Что запущено:** Postgres 17, бэкенд (готовый образ), фронтенд (статика за своим внутренним Caddy),
  Caddy (HTTPS, маршрутизирует `/api`, `/actuator`, `/swagger-ui`, `/v3/api-docs` на бэкенд, остальное —
  на фронтенд). Наружу открыты только порты 80 и 443, остальные контейнеры снаружи недоступны.
