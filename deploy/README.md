...
```
merge в main ──► CI тестирует ML и собирает образы ML, бэкенда и фронтенда ──► GitHub Container Registry
                                                        │
...
- **Что запущено:** Postgres 17, ML-сервис, бэкенд (готовый образ), фронтенд (статика за своим внутренним Caddy),
...
Сначала дождитесь успешного CI для этого коммита: он публикует три образа с одинаковым SHA.
Deploy временно входит в GHCR с токеном текущего запуска, загружает образы,
ожидает healthy от ML и проверяет его `/health` из контейнера. Поэтому даже новый
приватный GHCR-пакет ML доступен деплою без отдельного PAT на сервере.
Backend запускается в HTTP-режиме и обращается к `http://ml:8000`; наружу ML-порт не открыт.

ML использует артефакт и карту остановок, встроенные в образ. Доступны девять маршрутов.
Для остановок сейчас применяется равномерное демо-распределение маршрутного прогноза;
оно не является обученной оценкой посадок на конкретной остановке. Прогнозы после 2025 года
помечаются как сценарные: качество на этих датах не измерено. Полный период запроса должен
помещаться в диапазон 01.11.2025–31.12.2026; январь 2027 и более поздние даты требуют
нового артефакта и расширения календаря ML. Подробности: [`ml_readme.md`](../ml_readme.md).
...
docker compose -f docker-compose.prod.yml logs -f ml         # логи ML
